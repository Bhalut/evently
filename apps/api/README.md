...
| `PORT`         | API server port                          | `3001` (production) or `3000` (development) |
...
**Note**: For local development with CLI demo, the API may run on port 3000. The CLI is port-aware and will auto-detect the correct port.
...
**Note**: The `name` field was added to support user registration with full name.
...
# Using TypeORM CLI
typeorm-ts-node-commonjs migration:generate -d src/database/data-source.ts
```
...
If migrations fail, you can manually manage the database:

```bash
# Connect to database
docker compose exec -it db psql -U postgres -d evently
...
**Note**: The `name` field is required for user registration.
